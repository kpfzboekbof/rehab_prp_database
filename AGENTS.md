...
- **Performance budget for any page that hits the database: ≤ 3 Prisma round-trips, then cache.** Neon over the network adds ~80-150ms per query and the latency does NOT decrease with parallelism beyond a small fan-out; pages that fire 8+ queries per request feel sluggish (~1.5-2s) and pages that fire 40+ feel broken (~4-5s). When you find yourself writing a per-month or per-doctor loop with `Promise.all` over Prisma calls, STOP and rewrite as one of:
  - **Raw SQL with CTEs / GROUPING SETS / window functions** via `db.$queryRaw(Prisma.sql\`…\`)`. Postgres can compute totals + per-doctor + per-product breakdowns in a single round trip; we already do this in `src/server/queries/insights.ts::computeMonthlyTrend` (one CTE replaces 48 queries) and `src/server/queries/reports.ts::computeMonthlyReport` (GROUPING SETS replaces 8 queries).
  - **Single aggregate with post-processing in JS.** One `findMany` of a few hundred rows + JS reduction beats N Prisma calls every time.
  - **Snapshot-friendly cache via `unstable_cache`.** Anything that aggregates over `TreatmentRecord` snapshot fields (totalAmount / commissionAmount) is essentially immutable for past dates — cache it aggressively (5 min default, longer if you can). Cache keys auto-derive from positional args, so `getMonthlyReport({ year, month, doctorId })` caches each (year, month, doctorId) tuple independently.
  - **For server-action read-your-own-writes**, use `updateTag(tag)` (Next.js 16 primitive, single arg, server-action only) immediately after the write. NOT `revalidateTag` — in v16 that requires a profile arg and is meant for fetch-style caches. See `src/server/actions/treatments.ts` calling `updateTag("monthly-report")` after create/update.
- **Vercel Hobby plan limitations that have already bitten us once.** When the `vercel.json` cron schedule violates Hobby plan limits, Vercel will silently reject ALL deployments for the project — not even a "Failed" entry shows up in the Deployments list, the build just never starts. The blast radius is the entire project, not just the cron. If you need scheduled work on a Hobby plan, do NOT add a `crons` block to `vercel.json` more frequent than once per day; instead use an external pinger (UptimeRobot / cron-job.org / GitHub Actions cron) hitting `/api/ping` or whatever endpoint you need. The endpoint stays in this repo, the schedule lives outside Vercel.
